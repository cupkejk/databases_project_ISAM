...
class Manager:
        ...
        if self.adding_page.len() >= limit:
            if does_next_exist:
                ...
                if self.adding_page.recs[-1].key > rec.key:
                    if self.adding_page.recs[0].key > rec.key:
                        largest_rec = self.adding_page.recs.pop()
                        self.adding_page.recs.insert(0, rec)
                        self.adding_page.dirty = True
                        rec = largest_rec 
    ...
    def add_overflow(self, rec):
        ...
        if overflow == None:
            ...
            # [FIXED] REMOVED THE BAD IF/ELSE CHECK HERE
            self.adding_page.dirty = True 
            ...
            if new_overflow_rec.key > rec.key:
                ...
                # [FIXED] ENSURED DIRTY BIT IS SET CORRECTLY
                if i == 0:
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if choice == '1':
            ...
            try:
                m = Manager()
                n_recs = int(input("Enter the amount of records you want to add:"))
                start_time = time.time()
                m.experiment(n_recs)
                print(f"Experiment complete in {time.time() - start_time:.4f}s.")
                print(f"Reads after experiment: {m.reads}")
                print(f"Writes after experiment: {m.writes}")
            except ValueError:
                print("Invalid input.")
